...
function preview_theme_ob_filter_callback( $matches ) {
	...
	if (
		( false !== strpos($matches[3], '/wpManut/') )
	||
...
function switch_theme( $stylesheet ) {
	...
	if ( is_admin() && false === get_option( 'theme_mods_' . $stylesheet ) ) {
		 ...
		 * the Customizer when accessing wpManut/widgets.php.
		 */
		if ( 'wp_ajax_customize_save' === current_action() ) {
...
function _custom_header_background_just_in_time() {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		...
		if ( is_admin() ) {
			require_once( ABSPATH . 'wpManut/custom-header.php' );
	...
	if ( current_theme_supports( 'custom-background' ) ) {
		...
		if ( is_admin() ) {
			require_once( ABSPATH . 'wpManut/custom-background.php' );
 ...
 *
 * Fires when ?wp_customize=on or on wpManut/customize.php.
 *
 * @since 3.4.0
 */
