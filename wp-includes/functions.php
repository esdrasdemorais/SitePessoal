...
function wp_guess_url() {
	if ( defined('WP_SITEURL') && '' != WP_SITEURL ) {
		...
		if ( strpos( $_SERVER['REQUEST_URI'], 'wpManut' ) !== false || strpos( $_SERVER['REQUEST_URI'], 'wpL.php' ) !== false ) {
			$path = preg_replace( '#/(wpManut/.*|wpL.php)#i', '', $_SERVER['REQUEST_URI'] );
...
/**
 * Output the HTML that shows the wpL dialog when the user is no longer logged in.
 *
 * @since 3.6.0
 */
